...
export default function AdminReviews() {
  ...
  const fetchReviews = async () => {
    try {
      ...
      const res = await fetch("http://192.168.1.69:5000/api/admin/reviews", {
  ...
  const handleApprove = async (id) => {
    try {
      const token = localStorage.getItem("adminToken");
      const res = await fetch(`http://192.168.1.69:5000/api/admin/reviews/${id}/approve`, {
        method: "PATCH",
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem("adminToken");
      const res = await fetch(`http://192.168.1.69:5000/api/admin/reviews/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.ok) {
